...
double HungarianAlgorithm::Solve(vector <vector<double> >& DistMatrix, vector<int>& Assignment)
{
	...
	std::cout << "[HA.Solve] start nRows=" << nRows << " nCols=" << nCols << std::endl;

	double *distMatrixIn = new double[nRows * nCols];
	int *assignment = new int[nRows];
	double cost = 0.0;
	std::cout << "[HA.Solve] alloc OK, filling matrix" << std::endl;

	for (unsigned int i = 0; i < nRows; i++)
		for (unsigned int j = 0; j < nCols; j++)
			distMatrixIn[i + nRows * j] = DistMatrix[i][j];

	std::cout << "[HA.Solve] matrix filled, calling assignmentoptimal" << std::endl;
	assignmentoptimal(assignment, &cost, distMatrixIn, nRows, nCols);
	std::cout << "[HA.Solve] assignmentoptimal done cost=" << cost << std::endl;
	...
	delete[] distMatrixIn;
	delete[] assignment;
	std::cout << "[HA.Solve] done" << std::endl;
	return cost;
}
...
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	std::cout << "[HA.opt] start nRows=" << nOfRows << " nCols=" << nOfColumns << std::endl;
	...
	*cost = 0;
	...
	if (!distMatrix) { std::cerr << "[HA.opt] malloc distMatrix failed!" << std::endl; return; }
	distMatrixEnd = distMatrix + nOfElements;
	std::cout << "[HA.opt] distMatrix malloc OK n=" << nOfElements << std::endl;
	...
	for (row = 0; row<nOfElements; row++)
	{
		...
		if (!std::isfinite(value)) {
			std::cerr << "[HA.opt] NaN/Inf at index=" << row << std::endl;
			value = 1e9;
		}
		distMatrix[row] = value;
	}
	std::cout << "[HA.opt] matrix copy OK" << std::endl;
	...
	if (!coveredColumns || !coveredRows || !starMatrix || !primeMatrix || !newStarMatrix) {
		std::cerr << "[HA.opt] calloc failed!" << std::endl;
		free(distMatrix); return;
	}
	std::cout << "[HA.opt] calloc OK, starting reduction" << std::endl;
	...
	std::cout << "[HA.opt] before step2b" << std::endl;
	/* move to step 2b */
	step2b(assignment, distMatrix, starMatrix, newStarMatrix, primeMatrix, coveredColumns, coveredRows, nOfRows, nOfColumns, minDim);
	std::cout << "[HA.opt] after step2b, computing cost" << std::endl;
